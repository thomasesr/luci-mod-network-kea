...
//patched file functions
function isKeaSubnetEnabled(s) {
	return s && s['.type'] === 'subnet4' && s.enabled !== '0';
}

function normalizeIfname(ifname) {
	return (ifname != null) ? String(ifname).trim() : '';
}

function keaOwnsInterface(ifname) {
	ifname = normalizeIfname(ifname);

	if (!ifname)
		return false;

	return uci.sections('kea', 'subnet4').some(function(s) {
		return isKeaSubnetEnabled(s) && normalizeIfname(s.interface) === ifname;
	});
}
// end of patched functions
		...
		s.addModalOptions = function(s) {
			...
			return network.getNetwork(s.section).then(L.bind(function(ifc) {
				...
				if (L.hasSystemFeature('dnsmasq') || L.hasSystemFeature('odhcpd')) {
					//patched kea dhcp
					var keaManaged = keaOwnsInterface(ifc.getName());
					s.tab('dhcp', _('DHCP Server'));

					if (keaManaged) {
						let o;
					
						o = s.taboption('dhcp', form.DummyValue, '_kea_managed_notice', _('Kea DHCP'));
						o.modalonly = true;
						o.rawhtml = true;
						o.cfgvalue = function() {
							return '<div class="alert-message notice"><strong>%s</strong><br />%s</div>'.format(
								_('DHCP managed by Kea'),
								_('This interface is managed by Kea DHCP. Configure it under Services → Kea DHCP.')
							);
						};
					
						o = s.taboption('dhcp', form.Button, '_open_kea');
						o.modalonly = true;
						o.inputtitle = _('Open Kea DHCP');
						o.inputstyle = 'action';
						o.onclick = function() {
							window.location.href = L.url('admin/services/kea');
						};
